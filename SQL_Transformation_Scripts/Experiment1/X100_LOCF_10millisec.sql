...
DECLARE
    ...
    end_timestamp TIMESTAMP := '2024-11-10 04:00:10.999+011'; --last timestamp in the timeseries that has to be transformed 
    original_id VARCHAR := 'Exp3P1V5happiness_x100_with_gaps'; -- Set the original timeseries_id 
    ...
    time_interval INTERVAL := interval '10 millisecond'; -- Set the time interval 
    interpolation BOOLEAN := FALSE; --can be true of false
    LOCF BOOLEAN := TRUE; --can be true of false
BEGIN
    ...
    WITH new_timeseries AS (
        ...
        SELECT 
	    id || 'LOCF_gap_fill_10mlsec', -- Concatenate to the ID
            id,
            type,              
            measure_id,                    
            source,      
...
-- DELETE FROM query_execution_log where query_name LIKE 'x10%';